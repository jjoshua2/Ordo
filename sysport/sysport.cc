...
extern int /* boolean */
mythread_create(/*@out@*/ mythread_t* thread, routine_t start_routine,
                void* arg, /*@out@*/ int* ret_error) {
  // create joinable threads
  pthread_attr_t attr; /* attributes */
  int ret;
  pthread_attr_init(&attr);  // for portability
  pthread_attr_setdetachstate(&attr,
                              PTHREAD_CREATE_JOINABLE);  // for portability
...
void mythread_spinx_init(mythread_spinx_t* lock) {
...
void mythread_spinx_lock(mythread_spinx_t* lock) {
  int acquired;
  unsigned i = 0;
  do {
...
void mythread_spinx_unlock(mythread_spinx_t* lock) {
  __asm__ __volatile__("" ::: "memory");
  *lock = 0;
}
...
void semaphore_system_init(void) {
  mythread_spinx_init(&Semcounterlock);
  return;
}
void semaphore_system_done(void) {
  mythread_spinx_destroy(&Semcounterlock);
  return;
}
...
extern int /* boolean */
mysem_wait(mysem_t* sem) {
  return 0 == sem_wait(sem->psem);
}

extern int /* boolean */
mysem_post(mysem_t* sem) {
  return 0 == sem_post(sem->psem);
}

extern int /* boolean */
mysem_destroy(mysem_t* sem) {
  return 0 == sem_close(sem->psem) && 0 == sem_unlink(sem->name);
}

#elif defined(MY_SEMAPHORES)

static void condvar_init(condvar_t* pcv) {
  pcv->go = 0;
  pthread_cond_init(&pcv->key, NULL);
  pthread_mutex_init(&pcv->mtx, NULL);
}

static void condvar_done(condvar_t* pcv) {
  pcv->go = 0;
  pthread_cond_destroy(&pcv->key);
  pthread_mutex_destroy(&pcv->mtx);
}

static void condvar_release(condvar_t* pcv) {
...
static void condvar_wait(condvar_t* pcv) {
