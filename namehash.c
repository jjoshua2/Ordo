...
struct NAMEPEA {
  ...
  player_t
      pidx_out;  // player index to be used for synonyms, if different from pidx
  uint32_t hash;  // name hash
};

static bool_t name_tree_init(void);
static void name_tree_done(void);
static bool_t name_ispresent_hashtable(const struct DATA* d, const char* s,
                                       uint32_t hash,
                                       /*out*/ player_t* out_index);
static bool_t name_register_hashtable(uint32_t hash, player_t i,
                                      player_t i_out);
static bool_t name_ispresent_tree(const struct DATA* d, const char* s,
                                  uint32_t hash, /*out*/ player_t* out_index);
static bool_t name_register_tree(uint32_t hash, player_t i, player_t i_out);

//*************************** GENERAL **************************************

bool_t name_storage_init(void) { return TRUE; }

void name_storage_done(void) {
  name_tree_done();
  return;
}

bool_t name_ispresent(const struct DATA* d, const char* s, uint32_t hash,
                      /*out*/ player_t* out_index) {
  return name_ispresent_hashtable(d, s, hash, out_index) ||
         name_ispresent_tree(d, s, hash, out_index);
}

bool_t name_register(uint32_t hash, player_t i, player_t i_out) {
...
bool_t name_register_hashtable(uint32_t hash, player_t i, player_t i_out) {
  struct NAMEPOD* ppod = &Namehashtab[hash & PODMASK];
  struct NAMEPEA* ppea;
  int n;
...
static bool_t nodetree_is_hit(const struct DATA* d, const char* s,
...
static bool_t name_register_tree(uint32_t hash, player_t i, player_t i_out) {
  if (Treemembers == 0) name_tree_init();

  if (T_end == Tstop) {
    if (!name_tree_addmem()) return FALSE;
  }
  ...
  if (Treemembers > 0) nodetree_connect(Troot, T_end);
  Treemembers++;
  T_end++;
  return TRUE;
}
...
static bool_t nodetree_is_hit(const struct DATA* d, const char* s,
                              uint32_t hash, const struct NODETREE* pnode) {
  return hash == pnode->p.hash &&
         !strcmp(s, database_getname(d, pnode->p.pidx));
}
