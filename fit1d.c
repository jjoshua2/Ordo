...
static bool_t find_parabolic_min_x_withreference(const double* x,
                                                 const double* y,
                                                 double reference,
                                                 double* result) {
...
static bool_t find_parabolic_min_x(const double* x, const double* y,
                                   double* result) {
  double reference;
  bool_t ok;
  double res;
...
static double optimum_center(double* x, double* y) {
  double result;
  assert(x[3] > x[2]);
  assert(x[2] > x[1]);
  ...
  if ((x[3] - x[1]) > Epsilon && (x[2] - x[1]) > Epsilon &&
      (x[3] - x[2]) > Epsilon && find_parabolic_min_x(x, y, &result)) {
    assert(result > x[1] ||
...
double quadfit1d_2(double limit, double x1, double x2, double x3, double ya,
                   double yb, double yc,
  ...
  assert(!is_nan(limit));
  assert(!is_nan(x1) && !is_nan(x2) && !is_nan(x3) && !is_nan(ya) &&
         !is_nan(yb) && !is_nan(yc));
  assert(yc >= yb && ya >= yb);
  assert((x3 > x2 && x2 > x1) ||
         !fprintf(stderr, "x1=%lf, x2=%lf, x3=%lf\n", x1, x2, x3));
  ...
  do {
    ...
    if (equality) {
      ...
      } else if (x[3] - x[2] < (x[2] - x[1]) / 2) {  // upper third
