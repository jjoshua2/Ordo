...
#if 1
static double ind_expected(double x, double* rtng, double* weig, int n,
                           double beta) {
  int i;
  double cume = 0;
  for (i = 0; i < n; i++) {
...
static double adjust_x(double x, double xp, double sc, double delta,
                       double kappa) {
  double y;
  double d;
  d = xp - sc;
...
static double calc_ind_rating(double cume_score, double* rtng, double* weig,
                              int r, double beta) {
  int i;
...
static double prob2absolute_result(int perftype, double myrating, double* rtng,
                                   double* weig, int n, double deq,
                                   double beta) {
  int i;
  double p, cume;
...
static double calc_ind_rating_superplayer(int perf_type, double x_estimated,
                                          double* rtng, double* weig, int r,
                                          double deq, double beta) {
  int i;
...
static void rate_super_players_internal(bool_t quiet, struct ENC* enc,
                                        size_t N_enc, int* performance_type,
                                        player_t n_players, double* ratingof,
                                        double white_advantage, bool_t* flagged,
                                        const char* Name[], double deq,
                                        double beta, struct ENC* myenc,
                                        double weig[], double rtng[]) {
  size_t e;
  player_t j;
  size_t myenc_n = 0;
  ...
  for (j = 0; j < n_players; j++) {
    ...
    if (!quiet) {
      if (0 == myenc_n) {
        ...
        printf("  all wins   --> %s\n", Name[j]);
      } else if (performance_type[j] == PERF_SUPERLOSER) {
        printf("  all losses --> %s\n", Name[j]);
      }
    }
    ...
    if (0 != myenc_n) {
      ...
      if (performance_type[j] == PERF_SUPERWINNER) {
        double ori_estimation =
            calc_ind_rating(cume_score - 0.25, rtng, weig, r, beta);
        assert(r);
        ratingof[j] = calc_ind_rating_superplayer(
            PERF_SUPERWINNER, ori_estimation, rtng, weig, r, deq, beta);
      }
      if (performance_type[j] == PERF_SUPERLOSER) {
        ...
        assert(r);
        ratingof[j] = calc_ind_rating_superplayer(
            PERF_SUPERLOSER, ori_estimation, rtng, weig, r, deq, beta);
      }
      flagged[j] = FALSE;
    } else {
...
//
void rate_super_players(bool_t quiet, struct ENC* enc, gamesnum_t N_enc,
                        int* performance_type, player_t n_players,
                        double* ratingof, double white_advantage,
                        bool_t* flagged, const char* Name[], double deq,
                        double beta) {
    ...
    if (NULL != (rtng = memnew(sizeof(double) * ne))) {
      if (NULL != (myenc = memnew(sizeof(struct ENC) * ne))) {
        rate_super_players_internal(
            quiet, enc, ne, performance_type, (player_t)np, ratingof,
            white_advantage, flagged, Name, deq, beta, myenc, weig, rtng);
