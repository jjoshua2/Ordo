...
static int findidx(const struct option* longopts, char* a, int* longindex) {
  int i, found = 0;
  if (NULL == a || NULL == longopts) return 0;

  for (i = 0; longopts[i].name != NULL; i++) {
    found = a == strstr(a, longopts[i].name) &&
            terminates(a[strlen(longopts[i].name)]);
    if (found) {
      *longindex = i;
      break;
    }
...
static int optl(int argc, char* const argv[], const struct option* longopts,
                int* longindex) {
  int letter;
  int i;
