...
static bool_t eo_helplist(struct helpline* h) {
  return NULL == h->longc && '\0' == h->c;
}

struct option* optionlist_new(struct helpline* hl) {
  size_t n;
...
void optionlist_done(struct option* o) {
  assert(o);
  free(o);
}

char* optionshort_build(struct helpline* hl, char* opt_input) {
  char* opt = opt_input;
  struct helpline* i = hl;
...
static void build_head(char* head, struct helpline* h) {
  sprintf(head, " %c%c%s %s%s%s%s%s%s", h->c != '\0' ? '-' : ' ',
          h->c != '\0' ? h->c : ' ',
          h->longc != NULL && h->c != '\0' ? "," : "",  ////
          h->longc != NULL ? (h->c != '\0' ? "--" : " --") : "",
...
void printlonghelp(FILE* outf, struct helpline* h_inp) {
  struct helpline* h;
  char head[80];
  size_t longest = 0;
  int left_tab;
