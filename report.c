...
static int compare__(const player_t* a, const player_t* b,
...
static void insertion_sort(const double* reference, size_t n, player_t* vect) {
  size_t i, j, pivot;
  player_t tmp;
  if (n < 2) return;
  for (j = n - 1; j > 0; j--) {
...
static size_t partition(const double* reference, player_t* vect, size_t l,
                        size_t h) {
  size_t p;
  size_t i = l + 1;
  player_t tmp;
...
static void range_qsort(const double* reference, player_t* vect, size_t l,
                        size_t h) {
  size_t m;
  assert(h >= l);
  ...
  if (h <= l + 12) {
    ...
    if (0 < compare__(&vect[h], &vect[j], reference) &&
        0 < compare__(&vect[j], &vect[l], reference)) {
      tmp = vect[j];
      vect[j] = vect[l];
      vect[l] = tmp;  // swap
    }
    if (0 < compare__(&vect[j], &vect[h], reference) &&
        0 < compare__(&vect[h], &vect[l], reference)) {
      tmp = vect[h];
      vect[h] = vect[l];
      vect[l] = tmp;  // swap
    }
...
static void my_qsort(const double* reference, size_t n, player_t* vect) {
  range_qsort(reference, vect, 0, n - 1);
  insertion_sort(reference, n,
                 vect);  // only if it was not sorted inside range_qsort
}
...
static const char* get_super_player_symbolstr(player_t j,
...
static bool_t is_old_version(player_t j, const struct rel_prior_set* rps) {
  player_t i;
  bool_t found;
  player_t rn = rps->n;
...
static double rating_round(double x, int d) {
...
static char* get_sdev_str(double sdev, double confidence_factor, char* str,
                          int decimals) {
...
static char* get_sdev_str_csv(double sdev, double confidence_factor, char* str,
                              int decimals) {
  double x = sdev * confidence_factor;
  if (sdev > 0.00000001) {
    sprintf(str, "%.*f", decimals, rating_round(x, decimals));
...
static bool_t ok_to_out(player_t j, const struct output_qualifiers outqual,
                        const struct PLAYERS* p, const struct RATINGS* r) {
  gamesnum_t games = r->playedby_results[j];
  bool_t ok = !p->flagged[j] && games > 0 &&
...
void cegt_output(
    bool_t quiet, const struct GAMES* g, const struct PLAYERS* p,
    const struct RATINGS* r,
    struct ENCOUNTERS* e  // memory just provided for local calculations
    ,
    double* sdev, long simulate, double confidence_factor,
    const struct GAMESTATS* pgame_stats, const struct DEVIATION_ACC* s,
    struct output_qualifiers outqual, int decimals) {
  struct CEGT cegt;
  player_t j;
...
static const char* Header_INI[MAX_prnt] = {
    "PLAYER", "RATING", "ERROR", "POINTS", "PLAYED", "(%)",  "CFS(%)", "W",
    "D",      "L",      "D(%)",  "OppAvg", "OppErr", "OppN", "OppDiv"};
...
void report_columns_unset(int i) {
...
bool_t report_columns_set(int i, int shft, const char* hdr) {
  char* h;
...
bool_t report_columns_set_str(const char* i_str, const char* s_str,
                              const char* hdr) {
  int int1 = 0, int2 = 0;
  bool_t ok = TRUE;
  ...
  ok = ok && report_columns_set(
                 int1, ((size_t)int2 < min_len ? (int)min_len : int2), hdr);
  return ok;
}

bool_t report_columns_init(void) {
  bool_t ok = TRUE;
  int i;
...
void head2head_output(
    const struct GAMES* g, const struct PLAYERS* p, const struct RATINGS* r,
    struct ENCOUNTERS* e  // memory just provided for local calculations
    ,
    double* sdev, long simulate, double confidence_factor,
    const struct GAMESTATS* pgame_stats, const struct DEVIATION_ACC* s,
    const char* head2head_str, int decimals) {
  struct CEGT cegt;
  player_t j;
...
#ifndef NDEBUG
static bool_t is_empty_player(player_t j, const struct PLAYERS* pPlayers) {
  assert(pPlayers->perf_set);
  return pPlayers->performance_type[j] == PERF_NOGAMES;
}
#endif

static double get_cfs(const struct DEVIATION_ACC* sim, double dr,
                      player_t target, player_t oth) {
  double ret;
  if (sim != NULL) {
    ptrdiff_t idx;
    double ctrs;
...
static char* get_rank_str(int rank, char* buffer) {
  sprintf(buffer, "%d", rank);
  return buffer;
}

static char* quoted_str(const char* s, char* buffer) {
  sprintf(buffer, "\"%s\"", s);
  return buffer;
}
...
static size_t stripped_len(const char* s) {
  const char *p, *q;
  p = s;
  q = s + strlen(s);
  while (isspace(*p)) p++;
...
static void prnt_singleitem_(int item, size_t x, player_t j,
                             const struct REPORT_INFO* p_rprt_info, FILE* f,
                             size_t* plen) {
  ...
  sdev_str = sdev ? get_sdev_str(sdev[j], confidence_factor, sdev_buf, decimals)
                  : NOSDEV;
  oerr_str =
      sdev ? get_sdev_str(oi[j].opperr, confidence_factor, oerr_buf, decimals)
           : NOSDEV;
  ...
  switch (item) {
    case 0:
      sprintf(s, "%*s %-*s %s :", 4, rank_str, (int)ml + 1, p->name[j],
              get_super_player_symbolstr(j, p));
      break;
    case 1:
      sprintf(s, " %*.*f", sh, decimals,
              rating_round(r->ratingof_results[j], decimals));
      break;
    case 2:
      sprintf(s, " %*s", sh, sdev_str);
      break;
    case 3:
      sprintf(s, " %*.1f", sh, r->obtained_results[j]);
      break;
    case 4:
      sprintf(s, " %*ld", sh, (long)r->playedby_results[j]);
      break;
    case 5:
      sprintf(s, " %*.*f", sh, decperc,
              r->playedby_results[j] == 0 ? 0
                                          : 100.0 * r->obtained_results[j] /
                                                (double)r->playedby_results[j]);
      break;
    case 6:
      sprintf(s, " %*s", sh, cfs_str);
      break;
    case 7:
      sprintf(s, " %*ld", sh, (long)oi[j].W);
      break;
    case 8:
      sprintf(s, " %*ld", sh, (long)oi[j].D);
      break;
    case 9:
      sprintf(s, " %*ld", sh, (long)oi[j].L);
      break;
    case 10:
      sprintf(s, " %*.*f", sh, decperc,
              0 == oi[j].D ? 0.0
                           : (100.0 * (double)oi[j].D /
                              (double)(oi[j].W + oi[j].D + oi[j].L)));
      break;
    case 11:
      sprintf(s, " %*.*f", sh, decimals, oi[j].opprat);
      break;
    case 12:
      sprintf(s, " %*s", sh, oerr_str);
      break;
    case 13:
      sprintf(s, " %*ld", sh, (long)oi[j].n_opp);
      break;
    case 14:
      sprintf(s, " %*.*f", sh, 1, oi[j].diversity);
      break;
    default:
      break;
  }
...
static void prnt_itemlist_(int* list, size_t x, player_t j,
                           const struct REPORT_INFO* p_rprt_info, FILE* f) {
  int item, i;
  for (i = 0; list[i] != -1; i++) {
    item = list[i];
...
static void prnt_header_single(int item, FILE* f, size_t ml) {
  int sh;
  assert(item < MAX_prnt);
  sh = Shift[item];
  if (item >= MAX_prnt) return;
  if (item == 0) {
...
static void prnt_header(FILE* f, size_t ml, int* list) {
  int item;
  for (item = 0; list[item] != -1; item++)
    prnt_header_single(list[item], f, ml);
}

static void prnt_header_single_csv(int item, FILE* f) {
  char buffer[80];
  assert(item < MAX_prnt);
  if (item >= MAX_prnt) return;
  if (item == 0) {
...
static void prnt_header_csv(FILE* f, int* list) {
  int item;
  for (item = 0; list[item] != -1; item++)
    prnt_header_single_csv(list[item], f);
}

static void prnt_singleitem_csv(int item, FILE* f, int decimals,
                                const struct PLAYERS* p,
                                const struct RATINGS* r, player_t j, int rank,
                                const char* sdev_str, const char* oerr_str,
                                const char* cfs_str,
                                const struct OUT_INFO* oi) {
  assert(item < MAX_prnt);
  switch (item) {
    case 0:
      ...
      break;
    case 1:
      fprintf(f, ",%.*f", decimals, r->ratingof_results[j]);
      break;
    case 2:
      fprintf(f, ",%s", sdev_str);
      break;
    case 3:
      fprintf(f, ",%.2f", r->obtained_results[j]);
      break;
    case 4:
      fprintf(f, ",%ld", (long)r->playedby_results[j]);
      break;
    case 5:
      fprintf(f, ",%.2f",
              r->playedby_results[j] == 0 ? 0
    ...
    case 6:
      ...
      break;
    case 7:
      fprintf(f, ",%ld", (long)oi[j].W);
      break;
    case 8:
      fprintf(f, ",%ld", (long)oi[j].D);
      break;
    case 9:
      fprintf(f, ",%ld", (long)oi[j].L);
      break;
    case 10:
      fprintf(f, ",%.1f",
              0 == oi[j].D ? 0.0
      ...
      break;
    case 11:
      fprintf(f, ",%.*f", decimals, oi[j].opprat);
      break;
    case 12:
      fprintf(f, ",%s", oerr_str);
      break;
    case 13:
      fprintf(f, ",%ld", (long)oi[j].n_opp);
      break;
    case 14:
      fprintf(f, ",%.*f", 1, oi[j].diversity);
      break;
    default:
      break;
  }
...
static void prnt_itemlist_csv(FILE* f, int decimals, const struct PLAYERS* p,
                              const struct RATINGS* r, player_t j, int rank,
                              double* sdev, int* list, struct outextra* pq,
                              size_t x, double confidence_factor,
                              const struct OUT_INFO* oi) {
  int item;
  ...
  sdev_str =
      sdev ? get_sdev_str_csv(sdev[j], confidence_factor, sdev_buf, decimals)
           : NOSDEV_csv;
  oerr_str = sdev ? get_sdev_str_csv(oi[j].opperr, confidence_factor, oerr_buf,
                                     decimals)
                  : NOSDEV_csv;

  for (item = 0; list[item] != -1; item++)
    prnt_singleitem_csv(list[item], f, decimals, p, r, j, rank, sdev_str,
                        oerr_str, cfs_str, oi);
}

static void list_remove_item(int* list, int x);

static size_t listlen(int* x) {
  int* ori = x;
  while (*x != -1) x++;
  return (size_t)(x - ori);
}

static void listcopy(const int* s, int* t) {
  while (*s != -1) *t++ = *s++;
  *t = -1;
}

static void addabsent(int* list_chosen, int a) {
  int z;
...
static size_t find_maxlen(const char* nm[], struct outextra* q, size_t x_max) {
  size_t maxl = 0;
  size_t length;
  size_t x;
  player_t j;
...
void all_report(
    ...
    struct ENCOUNTERS* e  // memory just provided for local calculations
    ,
    double* sdev, long simulate, bool_t hide_old_ver, double confidence_factor,
    FILE* csvf, FILE* textf, double white_advantage, double drawrate_evenmatch,
    int decimals, int decimals_score, struct output_qualifiers outqual,
    double wa_sdev, double dr_sdev, const struct DEVIATION_ACC* s,
    bool_t csf_column, int* inp_list) {
  struct outextra* q;
  ...
  if (simulate < 2) list_remove_item(list_chosen, 2);
    ...
    if (ok_to_out(j, outqual, p, r)) {
      ...
      if (showrank || !hide_old_ver) {
        if (x > 0 && s && simulate > 1) {
          ...
          double delta_rating =
              r->ratingof_results[prev_j] - r->ratingof_results[j];
  ...
  if (f != NULL) {
    ...
    for (x = 0; x < x_max; x++) {
      ...
      prnt_itemlist_csv(f, decimals, p, r, j, rank_int, sdev, list_chosen, q, x,
                        confidence_factor, out_info);
      fprintf(f, "\n");
    }
  }
...
bool_t str2list(const char* inp_str, int max, int* n, int* t) {
...
void errorsout(const struct PLAYERS* p, const struct RATINGS* r,
               const struct DEVIATION_ACC* s, const char* out,
               double confidence_factor) {
  FILE* f;
  ptrdiff_t idx;
  player_t y, x;
...
void ctsout(const struct PLAYERS* p, const struct RATINGS* r,
            const struct DEVIATION_ACC* s, const char* out) {
  FILE* f;
  ptrdiff_t idx;
  player_t y;
...
void look_at_individual_deviation(player_t n_players, const bool_t* flagged,
                                  struct RATINGS* rat, struct ENC* enc,
                                  gamesnum_t n_enc, double white_adv,
                                  double beta) {
  double accum = 0;
  double diff;
  player_t j;
  ...
  if (expected) {
    printf("\nResidues\n\n");
    calc_expected(enc, n_enc, white_adv, n_players, rat->ratingof, expected,
                  beta);
...
void look_at_predictions(gamesnum_t n_enc, const struct ENC* enc,
                         const double* ratingof, double beta, double wadv,
                         double dr0) {
  gamesnum_t e;
  player_t w, b;
  double W, D, L;
...
void report_columns_load_settings(bool_t quietmode, const char* finp_name) {
  FILE* finp;
  ...
  if (NULL != (finp = fopen(finp_name, "r"))) {
    ...
    while (line_success && NULL != fgets(myline, MAXSIZE_CSVLINE, finp)) {
      ...
      if (csv_line_init(&csvln, myline)) {
        line_success = csvln.n == 3 && report_columns_set_str(
                                           csvln.s[0], csvln.s[1], csvln.s[2]);
        csv_line_done(&csvln);
      } else {
        line_success = FALSE;
      }
