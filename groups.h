...
extern player_t groupvar_build(group_var_t* gv, player_t N_plyers,
                               const char** name, const struct PLAYERS* players,
                               const struct ENCOUNTERS* encounters);
extern bool_t groupvar_init(group_var_t* gv, player_t nplayers,
                            gamesnum_t nenc);
extern void groupvar_done(group_var_t* gv);

extern group_var_t* GV_make(const struct ENCOUNTERS* encounters,
                            const struct PLAYERS* players);
extern group_var_t* GV_kill(group_var_t* gv);
extern void GV_out(group_var_t* gv, FILE* f);
extern void GV_sieve(group_var_t* gv, const struct ENCOUNTERS* encounters,
...
extern player_t GV_non_empty_groups_pop(group_var_t* gv,
                                        const struct PLAYERS* players);

extern bool_t well_connected(const struct ENCOUNTERS* pEncounters,
