...
struct DATA* database_init_frompgn(strlist_t* sl, const char* synfile_name,
                                   bool_t quiet) {
...
void database_done(struct DATA* p) {
  structdata_done(p);
  return;
}

const char* database_getname(const struct DATA* d, player_t i) {
...
void database_transform(const struct DATA* db, struct GAMES* g,
                        struct PLAYERS* p, struct GAMESTATS* gs) {
  ...
  assert(db && p && g && gs);
  assert(p->name && p->flagged && p->present_in_games && p->prefed &&
         p->priored && p->performance_type);
  ...
  gs->noresult = gamestat[DISCARD] + gamestat[IGNORED | WHITE_WIN] +
...
void database_ignore_draws(struct DATA* db) {
  size_t blk_filled = db->gb_filled;
  size_t idx_last = db->gb_idx;
  size_t blk;
  size_t idx;
...
void database_include_only(struct DATA* db, bitarray_t* pba) {
  player_t wp, bp;
  size_t blk;
  size_t idx;
  size_t blk_filled = db->gb_filled;
...
static const char* addname(struct DATA* d, const char* s) {
  char* b;
  char* nameptr;
  char* bf = NULL;
  namenode_t* nd;
...
static bool_t addplayer(struct DATA* d, const char* s, player_t* idx) {
  const char* nameptr;
...
static bool_t syn_apply_pair(struct DATA* d, const char* m, const char* s) {
#define NOPLAYER -1
  bool_t ok = TRUE;
...
static bool_t csvline_syn_apply(csv_line_t* pcsvln,
                                struct DATA* d /*, bool_t quiet*/) {
  bool_t ok = TRUE;
  int i;
...
static void syn_preload(bool_t quietmode, const char* fsyns_name,
                        struct DATA* d) {
  FILE* fsyns;
  ...
  if (NULL != (fsyns = fopen(fsyns_name, "r"))) {
    ...
    while (line_success && file_success &&
  ...
  if (!file_success) {
    ...
    if (!quietmode) printf("Synonyms uploaded succesfully\n");
  }
  if (!line_success) {
...
static bool_t name2player(const struct DATA* d, const char* namestr,
                          player_t* plyr) {
...
static bool_t do_tick(const struct DATA* d, const char* namestr,
                      bitarray_t* pba) {
...
static void warning(bool_t do_warning, const char* filename, const char* myline,
                    size_t linenumber) {
...
static bool_t isblankline(const char* s) {
  while (isspace(*s)) s++;
  return *s == '\0';
}

void namelist_to_bitarray(bool_t quietmode, bool_t do_warning,
                          const char* finp_name, const struct DATA* d,
                          bitarray_t* pba) {
  FILE* finp;
  char myline[MAXSIZE_CSVLINE];
  size_t linenumber = 0;
  ...
  if (!file_success) {
    ...
    fprintf(stderr,
            "Errors in file \"%s\", line %ld (line parsing problem or lack of "
            "memory)\n",
...
static void parsing_error(long line_counter) {
...
#define MAX_MYLINE 40000
  ...
  if (NULL == fpgn) return FALSE;

  if (!quiet) printf("Loading data (%d games x dot): \n\n", games_x_dot);
  fflush(stdout);
