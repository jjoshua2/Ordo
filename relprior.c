...
void relpriors_shuffle(struct rel_prior_set* rps /*@out@*/) {
  player_t i;
  double value, sigma;
  struct relprior* rp;
  player_t n;
...
void relpriors_copy(const struct rel_prior_set* r,
                    struct rel_prior_set* s /*@out@*/) {
  player_t i;
  player_t n = r->n;
...
void relpriors_show(const struct PLAYERS* plyrs,
  ...
  if (rn > 0) {
    ...
    for (i = 0; i < rn; i++) {
      ...
      printf("[%s] [%s] = %lf +/- %lf\n", plyrs->name[a], plyrs->name[b],
             rx[i].delta, rx[i].sigma);
    }
    printf("}\n");
  } else {
...
static void rpunit_build(int32_t p_a, int32_t p_b, double x, double sigma,
                         rpunit_t* u /*@out@*/) {
  u->player_a = p_a;
  u->player_b = p_b;
  u->delta = x;
...
static bool_t rman_set_relprior__(const struct PLAYERS* plyrs,
...
static bool_t rman_assign_relative_prior__(const struct PLAYERS* plyrs, char* s,
                                           char* z, double x, double y,
                                           bool_t quiet,
                                           struct rpmanager* rm /*@out@*/) {
  bool_t prior_success = TRUE;
  bool_t suc;

  if (y < 0) {
    fprintf(stderr,
            "Relative Prior, %s %s --> FAILED, error/uncertainty cannot be "
            "negative",
            s, z);
    suc = FALSE;
  } else {
    if (y < PRIOR_SMALLEST_SIGMA) {
      ...
      if (suc) {
        ...
        fprintf(stderr,
                "Relative Prior, %s, %s --> FAILED, name/s not found in input "
                "file\n",
                s, z);
      }
    }
...
void relpriors_init(bool_t quietmode, const struct PLAYERS* plyrs,
                    const char* f_name, struct rel_prior_set* rps /*@out@*/
                    ,
    ...
    if (rpman_init(&rpmanager)) {
      ...
      while (file_success && NULL != fgets(myline, MAX_MYLINE, fil)) {
        ...
        if (csv_line_init(&csvln, myline)) {
          success =
              csvln.n == 4 && getnum(csvln.s[2], &x) && getnum(csvln.s[3], &y);
          if (success) {
        ...
        prior_success = rman_assign_relative_prior__(plyrs, s, z, x, y,
                                                     quietmode, &rpmanager);
      }
...
void relpriors_done1(struct rel_prior_set* rps /*@out@*/) {
  assert(rps);
...
void relpriors_done2(struct rel_prior_set* rps /*@out@*/,
...
bool_t relpriors_replicate(struct rel_prior_set* rps,
                           struct rel_prior_set* rps_dup) {
  struct relprior* x;
...
void priors_reset(struct prior* p, player_t n) {
  player_t i;
  for (i = 0; i < n; i++) {
    p[i].value = 0;
    p[i].sigma = 1;
...
void priors_copy(const struct prior* p, player_t n, struct prior* q) {
  player_t i;
  for (i = 0; i < n; i++) {
    q[i] = p[i];
  }
...
void priors_shuffle(struct prior* p, player_t n) {
  player_t i;
  double value, sigma;
  for (i = 0; i < n; i++) {
    if (p[i].isset) {
...
void priors_show(const struct PLAYERS* plyrs, struct prior* p, player_t n) {
  player_t i;
  if (Priored_n > 0) {
    printf("Loose Anchors {\n");
    for (i = 0; i < n; i++) {
      if (p[i].isset) {
        printf("  [%s] = %lf +/- %lf\n", plyrs->name[i], p[i].value,
               p[i].sigma);
      }
    }
    printf("}\n");
...
static bool_t set_prior(const struct PLAYERS* plyrs, const char* player_name,
                        double x, double sigma, struct prior* pr) {
  player_t j;
  bool_t found;
  assert(sigma > PRIOR_SMALLEST_SIGMA);
...
// comes later
static bool_t set_anchor(const char* player_name, double x,
                         struct RATINGS* rat /*@out@*/,
                         struct PLAYERS* plyrs /*@out@*/);

static bool_t assign_prior(char* name_prior, double x, double y, bool_t quiet,
                           struct RATINGS* rat /*@out@*/,
                           struct PLAYERS* plyrs /*@out@*/,
                           struct prior* pr /*@out@*/) {
  bool_t prior_success = TRUE;
  ...
  if (y < 0) {
    fprintf(stderr,
            "Prior, %s --> FAILED, error/uncertainty cannot be negative",
            name_prior);
    suc = FALSE;
  } else {
    if (y < PRIOR_SMALLEST_SIGMA) {
      ...
      if (!suc) {
        ...
        if (!quiet) printf("Anchoring, %s --> %.1lf\n", name_prior, x);
      }
    } else {
...
void priors_load(bool_t quietmode, const char* fpriors_name,
                 struct RATINGS* rat /*@out@*/, struct PLAYERS* plyrs /*@out@*/,
                 struct prior* pr /*@out@*/) {
  FILE* fpriors;
  char myline[MAX_MYLINE];
  ...
  if (NULL != (fpriors = fopen(fpriors_name, "r"))) {
    ...
    while (file_success && NULL != fgets(myline, MAX_MYLINE, fpriors)) {
      ...
      if (csv_line_init(&csvln, myline)) {
        success =
            csvln.n >= 3 && getnum(csvln.s[1], &x) && getnum(csvln.s[2], &y);
        if (success) {
...
void anchor_j(player_t j, double x, struct RATINGS* rat /*@out@*/,
              struct PLAYERS* plyrs /*@out@*/) {
  plyrs->anchored_n++;
  plyrs->prefed[j] = TRUE;
  rat->ratingof[j] = x;
...
static bool_t assign_anchor(char* name_pinned, double x, bool_t quiet,
                            struct RATINGS* rat /*@out@*/,
                            struct PLAYERS* plyrs /*@out@*/) {
...
void init_manchors(bool_t quietmode, const char* fpins_name,
                   struct RATINGS* rat /*@out@*/,
                   struct PLAYERS* plyrs /*@out@*/) {
  FILE* fpins;
  char myline[MAX_MYLINE];
  ...
  if (NULL != (fpins = fopen(fpins_name, "r"))) {
    ...
    while (pin_success && file_success &&
           NULL != fgets(myline, MAX_MYLINE, fpins)) {
      success = FALSE;
      p = myline;
