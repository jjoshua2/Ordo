...
static double adjust_drawrate(double start_wadv, const double* ratingof,
                              gamesnum_t n_enc, const struct ENC* enc,
                              double beta);

static void ratings_copyto(player_t n_players, const double* r_fr,
                           double* r_to) {
  player_t j;
...
// no globals
static double deviation(player_t n_players, const bool_t* flagged,
                        const double* expected, const double* obtained,
...
// no globals
static double unfitness(const struct ENC* enc, gamesnum_t n_enc,
                        player_t n_players, const double* ratingof,
                        const bool_t* flagged, double white_adv, double beta,
                        const double* obtained, const gamesnum_t* playedby,
                        double* expected /*@out@*/
) {
  double dev;
...
static double calc_excess(player_t n_players, const bool_t* flagged,
                          double general_average, const double* ratingof) {
...
static void correct_excess(player_t n_players, const bool_t* flagged,
                           double excess, double* ratingof) {
  player_t j;
  for (j = 0; j < n_players; j++) {
...
// no globals
static double adjust_rating(double delta, double kappa, player_t n_players,
                            player_t anchored_n, const bool_t* flagged,
                            const bool_t* prefed, const double* expected,
                            const double* obtained, const gamesnum_t* playedby,
                            double* ratingof /*@out@*/
) {
  player_t j;
...
static void white_cal_obt_tot(gamesnum_t n_enc, const struct ENC* enc,
...
static double overallerrorE_fwadv(gamesnum_t n_enc, const struct ENC* enc,
                                  const double* ratingof, double beta,
                                  double wadv) {
  gamesnum_t tot;
  double cal, obt;  // calculated, obtained
  double dp2;
...
static double adjust_wadv(double start_wadv, const double* ratingof,
                          gamesnum_t n_enc, const struct ENC* enc, double beta,
                          double start_delta) {
  ...
  } while (delta > MIN_RESOL && -1000 < wa && wa < 1000);
...
static double overallerrorE_fdrawrate(gamesnum_t n_enc, const struct ENC* enc,
                                      const double* ratingof, double beta,
                                      double wadv, double dr0) {
  gamesnum_t e;
  player_t w, b;
  double dp2, f;
  double W, D, L;
...
static double adjust_drawrate(double start_wadv, const double* ratingof,
  ...
  do {
    ...
    if (ei >= ej && ej <= ek) {
      dr = quadfit1d(DRAWRATE_RESOLUTION, dr - delta, dr + delta,
                     unfit_drawrate, &p);
      break;
    } else if (ej >= ei && ei <= ek) {
      dr -= delta;
...
static void mobile_center_apply_excess(double excess, player_t n_players,
                                       const bool_t* flagged,
                                       const bool_t* prefed, double* ratingof) {
  player_t j;
...
static double unfitness_fcenter(double excess, const struct ENC* enc,
                                gamesnum_t n_enc, player_t n_players,
                                const double* ratingof, const bool_t* flagged,
                                const bool_t* prefed, double white_adv,
                                double beta, double* obtained, double* expected,
                                gamesnum_t* playedby, double* ratingtmp) {
  double u;
  assert(!is_nan(excess));
  ratings_copyto(n_players, ratingof, ratingtmp);
  mobile_center_apply_excess(excess, n_players, flagged, prefed, ratingtmp);
  u = unfitness(enc, n_enc, n_players, ratingtmp, flagged, white_adv, beta,
                obtained, playedby, expected);
  assert(!is_nan(u));
  return u;
}
...
static double unfitf(double x, const void* p) {
  ...
  r = unfitness_fcenter(x, q->enc, q->n_enc, q->n_players, q->ratingof,
                        ...
                        q->obtained, q->expected, q->playedby, q->ratingtmp);
  assert(!is_nan(r));
  return r;
}

static double optimum_centerdelta(double start_delta, double resolution,
                                  const struct ENC* enc, gamesnum_t n_enc,
                                  player_t n_players, const double* ratingof,
                                  const bool_t* flagged, const bool_t* prefed,
                                  double white_adv, double beta,
                                  double* obtained, double* expected,
                                  gamesnum_t* playedby, double* ratingtmp) {
  double lo_d, hi_d;
  struct UNFITPAR p;
  p.enc = enc;
...
static double get_outputdev(double curdev, gamesnum_t n_games) {
  return 1000 * sqrt(curdev / (double)n_games);
}

gamesnum_t calc_rating_ordo(bool_t quiet, bool_t adjust_white_advantage,
                            ...
                            ,
                            double BETA, double general_average, player_t anchor
  ...
  for (resol = START_RESOL, cycle = 0;
    ...
    olddev = curdev = unfitness(enc, n_enc, n_players, ratingof, flagged,
                                white_adv, BETA, obtained, playedby, expected);

    if (!quiet)
      printf("\nConvergence rating calculation (cycle #%d)\n\n", cycle + 1);
    if (!quiet)
      printf("%3s %4s %12s%14s\n", "phase", "iteration", "deviation",
             "resolution");
    ...
    while (!done && n-- > 0) {
      ...
      if (adjust_white_advantage) {
        ...
        wa_progress = wa_previous > white_adv ? wa_previous - white_adv
                                              : white_adv - wa_previous;
      ...
      for (i = 0; i < rounds && !done && !failed; i++) {
        ...
        resol = adjust_rating(delta, kappa * kk, n_players, anchored_n, flagged,
                              prefed, expected, obtained, playedby, ratingof);

        curdev = unfitness(enc, n_enc, n_players, ratingof, flagged, white_adv,
                           BETA, obtained, playedby, expected);
        failed = curdev >= olddev;

        if (failed) {
          ratings_copyto(n_players, ratingbk, ratingof);  // restore
          curdev = unfitness(enc, n_enc, n_players, ratingof, flagged,
                             white_adv, BETA, obtained, playedby, expected);
          assert(i == 0 || absol(curdev - olddev) < PRECISIONERROR ||
                 !fprintf(stderr,
                          "i=%d, curdev=%.10e, olddev=%.10e, diff=%.10e\n", i,
                          curdev, olddev, olddev - curdev));
        } else {
          cd = 0;  // includes the case (anchor_use && anchored_n == 1)
          if (anchored_n > 1) {
            cd = optimum_centerdelta(last_cd, MIN_RESOL, enc, n_enc, n_players,
                                     ratingof, flagged, prefed, white_adv, BETA,
                                     obtained, expected, playedby, ratingtmp);
          }
          last_cd = cd;

          if (absol(cd) > MIN_RESOL) {
            mobile_center_apply_excess(cd, n_players, flagged, prefed,
                                       ratingof);
          }
  ...
  rate_super_players(quiet, enc, n_enc, Performance_type, n_players, ratingof,
                     white_adv, flagged, name, draw_rate, BETA);
