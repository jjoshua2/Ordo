...
static void ratings_set_to(double general_average,
...
void get_a_simulated_run(int limit, bool_t quiet_mode, double beta,
                         double drawrate_evenmatch_result,
                         double white_advantage_result

                         ,
                         const struct RATINGS* pRA, const struct prior* PP_ori,
  ...
  do {
    ...
    simulate_scores(pRA->ratingof_results, drawrate_evenmatch_result,
                    white_advantage_result, beta, pGames /*out*/);
    ...
    encounters_calculate(ENCOUNTERS_FULL, pGames, pPlayers->flagged,
                         pEncounters);
    ...
    if (0 < players_set_super(quiet_mode, pEncounters, pPlayers)) {
      ...
      encounters_calculate(ENCOUNTERS_NOFLAGGED, pGames, pPlayers->flagged,
                           pEncounters);
    }
...
static int rand_threeway_wscore(double pwin, double pdraw) {
  long z, x, y;
...
void save_simulated(struct PLAYERS* pPlayers, struct GAMES* pGames, int num) {
  gamesnum_t i;
  const char* name_w;
  const char* name_b;
  const char* result;
...
static bool_t smpcount_get(long* x) {
  bool_t ok;
...
static void smpcount_set(long x) {
  mythread_mutex_lock(&Smpcount);
  Sim_N = x;
  mythread_mutex_unlock(&Smpcount);
}
...
static void ratings_set_to(double general_average,
  ...
  ratings_set(pPlayers->n, general_average, pPlayers->prefed, pPlayers->flagged,
              pRA->ratingof);
  ratings_set(pPlayers->n, general_average, pPlayers->prefed, pPlayers->flagged,
              pRA->ratingbk);
...
static void updates_print_scale(bool_t sim_updates) {
  if (sim_updates) {
...
static void updates_print_head(bool_t quiet, long z, long simulate) {
...
static void updates_print_progress(bool_t sim_updates) {
  double fraction;
  int astcount;
...
static void updates_print_reachedgoal(bool_t sim_updates) {
  int astcount;
  mythread_mutex_lock(&Printmtx);
  astcount = Astcount;
  if (sim_updates) {
...
void simul(long simulate, bool_t sim_updates, bool_t quiet_mode,
           bool_t prior_mode, bool_t adjust_white_advantage,
           bool_t adjust_draw_rate, bool_t anchor_use, bool_t anchor_err_rel2avg

           ,
           double general_average, player_t anchor, player_t priored_n,
           double beta

           ,
           double drawrate_evenmatch_result, double white_advantage_result,
           const struct rel_prior_set* rps, const struct prior* pPrior,
  ...
  while (smpcount_get(&zz)) {
    ...
    get_a_simulated_run(100, quiet_mode, beta, drawrate_evenmatch_result,
                        white_advantage_result, &RA, PP, &RPset,
                        &Encounters  // output
                        ,
                        &Players  // output
                        ,
    ...
    Encounters.n = calc_rating(quiet_mode, prior_mode, adjust_white_advantage,
                               ...
                               ,
                               &Encounters, &RPset_work, &Players, &RA, &Games

                               ,
                               PP_work, wa_prior, dr_prior
    ...
    mythread_mutex_lock(&Summamtx);
    summations_update(sfe, topn, RA.ratingof, white_advantage,
                      drawrate_evenmatch);
    mythread_mutex_unlock(&Summamtx);
...
void simul_smp(int cpus, long simulate, bool_t sim_updates, bool_t quiet_mode,
               bool_t prior_mode, bool_t adjust_white_advantage,
...
#define MAX_CPUS 64
    ...
    for (t = 0; t < CPUS; t++) {
      iret[t] =
          mythread_create(&threadid[t], simul_smp_process, pdata, &err[t]);
    }
...
static /*@null@*/
  ...
  simul(s->simulate, s->sim_updates, s->quiet_mode, s->prior_mode,
        s->adjust_white_advantage, s->adjust_draw_rate, s->anchor_use,
        s->anchor_err_rel2avg, s->general_average, s->anchor, s->priored_n,
        s->beta, s->drawrate_evenmatch_result, s->white_advantage_result,
        s->rps, s->pPrior, s->wa_prior, s->dr_prior
