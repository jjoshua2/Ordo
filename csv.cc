...
static void rm_blank_tails(char* s) {
  char* t = s + strlen(s);
...
static char* csv_gettoken(char* p, char* s, size_t max) {
  ...
  if (isquote(*p)) {
    p++;
    while (*p != '\0' && s < max + s_ori && !isquote(*p)) {
      *s++ = *p++;
    }
    *s = '\0';
    if (isquote(*p++)) {
...
bool_t csv_line_init(csv_line_t* c, char* p) {
  char* s;
  bool_t success;
...
void csv_line_done(csv_line_t* c) {
  assert(c != NULL);
  assert(c->mem != NULL);
