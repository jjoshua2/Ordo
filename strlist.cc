...
bool_t strlist_init(strlist_t* sl) {
...
static void freenode(strnode_t* n) {
  assert(n);
  if (n) {
...
bool_t strlist_push(strlist_t* sl, const char* s) {
  strnode_t* nn;
  char* ns;
  bool_t ok;
...
void strlist_done(strlist_t* sl) {
  strnode_t* ph;
  strnode_t* f;
...
void strlist_rwnd(strlist_t* sl) {
  if (sl) sl->curr = sl->prehead.nxt;
}

const char* strlist_next(strlist_t* sl) {
  char* rstr = NULL;
