...
bool_t players_name2idx(const struct PLAYERS* plyrs, const char* player_name,
                        player_t* pi) {
  player_t j;
  bool_t found;
...
void players_purge(bool_t quiet, struct PLAYERS* pl) {
...
void players_set_priored_info(const struct prior* pr,
                              const struct rel_prior_set* rps,
                              struct PLAYERS* pl /*@out@*/) {
  player_t i, j;
  player_t n_players = pl->n;
...
void players_flags_reset(struct PLAYERS* pl) {
  player_t j;
  player_t n;
  bool_t* flagged;
...
player_t players_set_super(bool_t quiet, const struct ENCOUNTERS* ee,
                           struct PLAYERS* pl) {
  // encounters
  ...
  for (e = 0; e < N_enc; e++) {
    ...
    assert((w >= 0 && w < n_players) ||
           !fprintf(stderr, "w=%ld np=%ld\n", (long)w, (long)n_players));
    assert((b >= 0 && b < n_players) ||
           !fprintf(stderr, "b=%ld np=%ld\n", (long)b, (long)n_players));
  ...
  for (j = 0; j < n_players; j++) {
    ...
    if (pla[j] == 0) {
      ...
      if (obt[j] < 0.001) {
        ...
        // if (!quiet) printf ("detected (all-losses player) --> %s: seed rating
        // present = %s\n", name[j], gotprior? "Yes":"No");
        counter_all_L++;
      }
      if ((double)pla[j] - obt[j] < 0.001) {
        perftype[j] = gotprior ? PERF_NORMAL : PERF_SUPERWINNER;
        // if (!quiet) printf ("detected (all-wins player)   --> %s: seed rating
        // present = %s\n", name[j], gotprior? "Yes":"No");
        counter_all_W++;
      }
    }
...
void players_copy(const struct PLAYERS* source, struct PLAYERS* target) {
...
#if !defined(NDEBUG)
bool_t players_have_clear_flags(struct PLAYERS* pl) {
  bool_t found;
  player_t j;
  player_t n_players;
  bool_t* flagged;
