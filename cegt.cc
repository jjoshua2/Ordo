...
static bool_t output_report_individual_f(FILE* indf, struct CEGT* p,
                                         int simulate);

static bool_t output_cegt_style_f(FILE* genf, FILE* ratf, FILE* prgf,
                                  struct CEGT* p);

static bool_t ok_to_out(const struct output_qualifiers* poutqual,
                        bool_t flagged, gamesnum_t games) {
  bool_t ok = !flagged && games > 0 &&
              (!poutqual->mingames_set || games >= poutqual->mingames);
  return ok;
}

extern bool_t output_cegt_style(bool_t quiet, const char* general_name,
                                const char* rating_name,
                                const char* programs_name, struct CEGT* p) {
  bool_t success;
  FILE *genf, *ratf, *prgf;
...
extern bool_t output_report_individual(const char* outindiv_name,
                                       struct CEGT* p, int simulate) {
  bool_t success;
  FILE* indivf;
  ...
  if (f != NULL) {
...

    {
      fprintf(f, "\n\n%s %-*s     %*s %*s %*s %7s %7s %8s %6s\n\n", "    ",
              (int)ml, "Program", 4 + extension, "Elo", 4 + extension, "+",
              4 + extension, "-", "Games", "Score", "Av.Op.", "Draws");
      ...
      for (i = 0; i < N_players; i++) {
        ...
        if (ok_to_out(&p->outqual, Flagged[j], Playedby_results[j])) {
          ...
          fprintf(
              f,
              "%4lu %-*s %s : %*.*f %*.*f %*.*f %5ld %7.1f%s %6.0f %6.1f%s\n",
              (long unsigned)rank, (int)ml + 1, Name[j],
              get_super_player_symbolstr(j, p), 4 + extension, decimals,
              Ratingof_results[j], 4 + extension, decimals,
              Sdev ? Sdev[j] * confidence : 0, 4 + extension, decimals,
              Sdev ? Sdev[j] * confidence : 0, (long)Playedby_results[j],
              Playedby_results[j] == 0
                  ? 0
                  : 100.0 * Obtained_results[j] / (double)Playedby_results[j],
              " %", av_opp(j, p), draw_percentage(j, Enc, N_enc), " %");
        }
      }
    }
...
static void all_report_prg(FILE* textf, struct CEGT* p, struct ENC* Temp_enc) {
  ...
  for (i = 0; i < N_players; i++) {
    ...
    if (!Flagged[j]) {
      ...
      fprintf(
          f, "%ld %-*s :%5.*f %ld (+%3ld,=%3ld,-%3ld), %4.1f %s\n\n",
          (long)i + 1, (int)(ml + (nlen - calclen((long)i + 1))), Name[target],
          decimals, Ratingof_results[target], (long)(won + dra + los),
      ...
      for (e = 0; e < t;) {
        player_t oth =
            Temp_enc[e].wh == target ? Temp_enc[e].bl : Temp_enc[e].wh;
        ...
        if ((e + 1) < t && Temp_enc[e].wh == Temp_enc[e + 1].bl &&
        ...
        fprintf(
            f, "%-*s : %ld (+%3ld,=%3ld,-%3ld), %4.1f %s\n",
            (int)(ml + nlen + 1), Name[oth], (long)(won + dra + los), (long)won,
            (long)dra, (long)los,
            100.0 * ((double)won + (double)dra / 2) / (double)(won + dra + los),
            "%");
        e++;
...
static ptrdiff_t head2head_idx_sdev(player_t inp_x, player_t inp_y) {
  ptrdiff_t idx;
...
static char* get_ratingstr(char* s, int decimals, double rating) {
  sprintf(s, "%.*f", decimals, rating);
  return s;
}

static size_t get_ratingstr_len(char* s, int decimals, double rating) {
  sprintf(s, "%.*f", decimals, rating);
  return strlen(s);
}
...
static void all_report_indiv_stats(FILE* textf, struct CEGT* p, int simulate,
  ...
  for (i = 0; i < N_players; i++) {
    ...
    if (!Flagged[j]) {
      ...
      fprintf(
          f, "%*ld) %-*s %*s : %*ld (+%*ld,=%*ld,-%*ld), %5.1f %s\n\n", indent,
          (long)i + 1, (int)ml, Name[target], (int)maxl, rstr, (int)gl,
          (long)(won + dra + los), gwlen, (long)won, gdlen, (long)dra, gllen,
          (long)los,
          100.0 * ((double)won + (double)dra / 2) / (double)(won + dra + los),
          "%");
      ...
      fprintf(f,
              ...
              //
              ,
              indent + 2, "", (int)ml + 6 + decimals, "vs.", " : ", (int)gl,
              "games"
      ...
      if (simulate > 1 && p->sim != NULL) {
        fprintf(f, ", %*s, %*s", 4 + decimals, "SD", 6, "CFS (%)");
      }
      ...
      for (e = 0; e < nl; e++) {
        ...
        fprintf(f,
                "%*s"
                "%-*s : %*ld ( %*ld, %*ld, %*ld), %5.1f",
                indent + 2, "", (int)ml + 6 + decimals, Name[oth], (int)gl,
                (long)(won + dra + los), gwlen, (long)won, gdlen, (long)dra,
                gllen, (long)los,
                100.0 * ((double)won + (double)dra / 2) /
                    (double)(won + dra + los));
...
static void all_report_gen(FILE* textf, struct CEGT* p) {
  ...
  fprintf(textf,
          ...
          ,
          (long)totalgames, (long)white_wins, 100 * white_perc, "%",
          (long)black_wins, 100 * black_perc, "%", (long)draw, 100 * draw_perc,
          "%", (long)p->gstat->noresult, 100 * white_performance, "%",
