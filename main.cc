...
static const char* intro_str = "Program to calculate individual ratings\n";

static const char* example_options = "-a 2500 -p input.pgn -o output.txt";
...
struct helpline SH[] = {
    ...
    {'H', "show-switches", no_argument, NULL, 0,
     "print switch information and exit"},
    {'v', "version", no_argument, NULL, 0, "print version number and exit"},
    {'L', "license", no_argument, NULL, 0,
     "print license information  and exit"},
    {'q', "quiet", no_argument, NULL, 0,
     "quiet mode (no progress updates on screen)"},
    {'\0', "silent", no_argument, NULL, 0, "same as --quiet"},
    {'Q', "terse", no_argument, NULL, 0,
     "same as --quiet, but shows simulation counter"},
    {'\0', "timelog", no_argument, NULL, 0,
     "outputs elapsed time after each step"},
    {'a', "average", required_argument, "NUM", 0,
     "set rating for the pool average"},
    {'A', "anchor", required_argument, "<player>", 0,
     "anchor: rating given by '-a' is fixed for <player>"},
    {'V', "pool-relative", no_argument, NULL, 0,
     "errors relative to pool average, not to the anchor"},
    {'m', "multi-anchors", required_argument, "FILE", 0,
     "multiple anchors: file contains rows of \"AnchorName\",AnchorRating"},
    {'y', "loose-anchors", required_argument, "FILE", 0,
     "loose anchors: file contains rows of \"Player\",Rating,Uncertainty"},
    {'r', "relations", required_argument, "FILE", 0,
     "relations: rows of \"PlayerA\",\"PlayerB\",delta_rating,uncertainty"},
    {'R', "remove-older", no_argument, NULL, 0,
     "no output of older 'related' versions (given by -r)"},
    {'w', "white", required_argument, "NUM", 0,
     "white advantage value (default=0.0)"},
    {'u', "white-error", required_argument, "NUM", 0,
     "white advantage uncertainty value (default=0.0)"},
    {'W', "white-auto", no_argument, NULL, 0,
     "white advantage will be automatically adjusted"},
    {'d', "draw", required_argument, "NUM", 0,
     "draw rate value % (default=50.0)"},
    {'k', "draw-error", required_argument, "NUM", 0,
     "draw rate uncertainty value % (default=0.0 %)"},
    {'D', "draw-auto", no_argument, NULL, 0,
     "draw rate value will be automatically adjusted"},
    {'z', "scale", required_argument, "NUM", 0,
     "set rating for winning expectancy of 76% (default=202)"},
    {'T', "table", no_argument, NULL, 0, "display winning expectancy table"},
    {'p', "pgn", required_argument, "FILE", 0, "input file, PGN format"},
    {'P', "pgn-list", required_argument, "FILE", 0,
     "multiple input: file with a list of PGN files"},
    {'o', "output", required_argument, "FILE", 0, "output file, text format"},
    {'c', "csv", required_argument, "FILE", 0,
     "output file, comma separated value format"},
    {'E', "elostat", no_argument, NULL, 0,
     "output files in elostat format (rating.dat, programs.dat & general.dat)"},
    {'j', "head2head", required_argument, "FILE", 0,
     "output file with head to head information"},
    {'g', "groups", required_argument, "FILE", 0,
     "outputs group connection info (no rating output)"},
    {'G', "force", no_argument, NULL, 0,
     "force program to run ignoring isolated-groups warning"},
    {'s', "simulations", required_argument, "NUM", 0,
     "perform NUM simulations to calculate errors"},
    {'e', "error-matrix", required_argument, "FILE", 0,
     "save an error matrix (use of -s required)"},
    {'C', "cfs-matrix", required_argument, "FILE", 0,
     "save a matrix (comma separated value .csv) with confidence for "
     "superiority (-s was used)"},
    {'J', "cfs-show", no_argument, NULL, 0,
     "output an extra column with confidence for superiority (relative to the "
     "player in the next row)"},
    {'F', "confidence", required_argument, "NUM", 0,
     "confidence to estimate error margins (default=95.0)"},
    {'X', "ignore-draws", no_argument, NULL, 0,
     "do not take into account draws in the calculation"},
    {'t', "threshold", required_argument, "NUM", 0,
     "threshold of games for a participant to be included"},
    {'N', "decimals", required_argument, "<a,b>", 0,
     "a=rating decimals, b=score decimals (optional)"},
    {'M', "ML", no_argument, NULL, 0,
     "force maximum-likelihood estimation to obtain ratings"},
    {'n', "cpus", required_argument, "NUM", 0,
     "number of processors used in simulations"},
    {'U', "columns", required_argument, "<a,..,z>", 0,
     "info in output (default columns are \"0,1,2,3,4,5\")"},
    {'Y', "synonyms", required_argument, "FILE", 0,
     "name synonyms (comma separated value format). Each line: main,syn1,syn2 "
     "or \"main\",\"syn1\",\"syn2\""},
    {'\0', "aliases", required_argument, "FILE", 0, "same as --synonyms FILE"},
    {'i', "include", required_argument, "FILE", 0,
     "include only games of participants present in FILE"},
    {'x', "exclude", required_argument, "FILE", 0,
     "names in FILE will not have their games included"},
    {'\0', "no-warnings", no_argument, NULL, 0,
     "supress warnings of names from -x or -i that do not match names in input "
     "file"},
    {'b', "column-format", required_argument, "FILE", 0,
     "format column output, each line form FILE being "
     "<column>,<width>,\"Header\""},
...
static bool_t validate_dec_array(int max, int decimals_array_n,
                                 int* decimals_array) {
  int i;
...
int main(int argc, char* argv[]) {
  ...
  const char *includes_str, *excludes_str, *columns_format_str, *multi_pgn,
      *single_pgn;
  int version_mode, help_mode, switch_mode, license_mode, input_mode,
      table_mode;
  bool_t group_is_output, Elostat_output, Ignore_draws, groupcheck, Forces_ML,
      cfs_column;
  bool_t switch_w = FALSE, switch_W = FALSE, switch_u = FALSE, switch_d = FALSE,
         switch_k = FALSE, switch_D = FALSE;
  ...
  while (END_OF_OPTIONS !=
         (op = options_l(argc, argv, OPTION_LIST, long_options, &longoidx))) {
    switch (op) {
      case '\0':
      ...
      case 'a':
        ...
        break;
      case 'F':
        if (1 != sscanf(opt_arg, "%lf", &Confidence) ||
        ...
        break;
      case 'A':
        if (strlen(opt_arg) < MAX_ANCHORSIZE - 1) {
        ...
        break;
      case 'V':
        Anchor_err_rel2avg = TRUE;
        break;
      case 'M':
        Forces_ML = TRUE;
        break;
      case 's':
        if (1 != sscanf(opt_arg, "%lu", &Simulate) || Simulate < 0) {
          fprintf(stderr, "wrong simulation parameter\n");
          exit(EXIT_FAILURE);
        }
        break;
      case 'w':
        if (1 != sscanf(opt_arg, "%lf", &White_advantage)) {
        ...
        break;
      case 'u':
        if (1 != sscanf(opt_arg, "%lf", &White_advantage_SD)) {
        ...
        break;
      case 'd':
        if (1 != sscanf(opt_arg, "%lf", &Drawrate_evenmatch_percent)) {
        ...
        if (Drawrate_evenmatch_percent >= 0.0 &&
            Drawrate_evenmatch_percent <= 100.0) {
      ...
      case 'k':
        if (1 !=
            sscanf(opt_arg, "%lf", &Drawrate_evenmatch_percent_SD)) {  // drsd
        ...
        break;
      case 'z':
        if (1 != sscanf(opt_arg, "%lf", &Rtng_76)) {
      ...
      case 'X':
        ...
        break;
      case 'N':
        output_decimals = opt_arg;
        break;
      case 't':
        if (1 != sscanf(opt_arg, "%ld", &mingames) || mingames < 0) {
        ...
        break;
      case 'n':
        if (1 != sscanf(opt_arg, "%d", &cpus) || cpus < 1) {
          fprintf(stderr, "wrong nuber of processors selected\n");
          exit(EXIT_FAILURE);
        }
        break;
      case 'U':
        output_columns = opt_arg;
        break;
      case 'Y':
        synstr = opt_arg;
        break;
      case 'i':
        includes_str = opt_arg;
        break;
      case 'x':
        excludes_str = opt_arg;
        break;
      case 'b':
        columns_format_str = opt_arg;
        break;
      case '?':
        parameter_error();
        exit(EXIT_FAILURE);
  ...
  if (output_columns != NULL) {
    if (!str2list(output_columns, COLSMAX, &columns_n, columns)) {
      fprintf(stderr,
              "Columns number provided are wrong or exceeded limit (%d)\n\n",
              COLSMAX);
      exit(EXIT_FAILURE);
    }
  } else {
    if (!str2list("0,1,2,3,4,5", COLSMAX, &columns_n, columns)) {
      fprintf(stderr,
              "Default number of columns is wrong or exceeded limit (%d)\n\n",
              COLSMAX);
      exit(EXIT_FAILURE);
    }
  }
  if (output_decimals != NULL) {
    if (!str2list(output_decimals, DECMAX, &decimals_array_n, decimals_array)) {
      fprintf(stderr,
              "Decimals number provided are wrong or exceeded limit (%d)\n\n",
              DECMAX);
      exit(EXIT_FAILURE);
    }
    if (!validate_dec_array(DECMAX, decimals_array_n, decimals_array) ||
        decimals_array_n == 0) {
    ...
    if (!str2list("1,0", DECMAX, &decimals_array_n, decimals_array)) {
      fprintf(stderr,
              "Default number of decimals is wrong or exceeded limit (%d)\n\n",
              DECMAX);
      exit(EXIT_FAILURE);
    }
    OUTDECIMALS = decimals_array[0];
  ...
  if (NULL != pinsstr && (General_average_set || Anchor_use)) {
    fprintf(stderr,
            "Setting a general average (-a) or a single anchor (-A) is "
            "incompatible with multiple anchors (-m)\n\n");
    exit(EXIT_FAILURE);
  }
  if ((switch_w || switch_u) && switch_W) {
    fprintf(stderr,
            "Switches -w/-u and -W are incompatible and will not work "
            "simultaneously\n\n");
    exit(EXIT_FAILURE);
  }
  if ((switch_d || switch_k) && switch_D) {  // drsd
    fprintf(stderr,
            "Switches -d/-k and -D are incompatible and will not work "
            "simultaneously\n\n");
    exit(EXIT_FAILURE);
  }
  if (NULL != priorsstr && General_average_set) {
    fprintf(stderr,
            "Setting a general average (-a) is incompatible with having a file "
            "with rating seeds (-y)\n\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (!name_storage_init()) {
    fprintf(stderr,
            "Problem initializing buffers for reading names, probably lack of "
            "memory.\n");
    exit(EXIT_FAILURE);
  }
  ...
  database_transform(pdaba, &Games, &Players,
                     &Game_stats); /* convert database to global variables */
  if (0 == Games.n) {
    fprintf(stderr, "ERROR: Input file contains no games\n");
    return EXIT_FAILURE;
  }
  ...
  if (Anchor_use) {
    ...
    if (players_name2idx(&Players, Anchor_name, &anch_idx)) {
      ...
      fprintf(stderr,
              "ERROR: No games of anchor player, mispelled, wrong capital "
              "letters, or extra spaces = \"%s\"\n",
              Anchor_name);
      fprintf(stderr,
              "Surround the name with \"quotes\" if it contains spaces\n\n");
      return EXIT_FAILURE;
    }
  }
  ...
  if (!(Drawrate_evenmatch > 0.0) && Game_stats.draws > 0 && Prior_mode) {
    fprintf(stderr,
            "ERROR: Draws present in the database but -d switch specified an "
            "invalid number\n");
    return EXIT_FAILURE;
  }
  ...
  if (Drawrate_evenmatch > 0.999) {
    fprintf(stderr,
            "ERROR: Draw rate set with -d switch is too high, > 99.9%s\n", "%");
    return EXIT_FAILURE;
  }
  ...
  if (group_is_output) {
    ...
    if (!quiet_mode) {
      printf("\nGroups=%ld\n", (long)GV_counter(gv));
      printf(
          "Encounters: Total=%ld, within groups=%ld, @ interface between "
          "groups=%ld\n",
    ...
    if (groups_n > 1) {
      ...
      fprintf(stderr,
              "********************[ WARNING ]**********************\n");
      fprintf(stderr,
              "*     Database is not well connected by games       *\n");
      fprintf(stderr,
  ...
  if (0 < players_set_super(quiet_mode, &Encounters, &Players)) {
    ...
    encounters_calculate(ENCOUNTERS_NOFLAGGED, &Games, Players.flagged,
                         &Encounters);
  }
  ...
  if (groupcheck && !well_connected(&Encounters, &Players)) {
    ...
    fprintf(stderr,
            "*       Database is not well connected by games...          *\n");
    fprintf(stderr,
            "* ...even after purging players with all-wins/all-losses    *\n");
    fprintf(stderr,
            "*                                                           *\n");
    fprintf(stderr,
            "*    Run switch -g to find what groups need more games      *\n");
    fprintf(stderr,
            "*   Run switch -G to ignore warnings and force calculation  *\n");
    fprintf(stderr,
            "*   (Attempting it may be very slow and may not converge)   *\n");
    fprintf(stderr,
            "*************************************************************\n");
    exit(EXIT_FAILURE);
  }
  ...
  Encounters.n =
      calc_rating(quiet_mode, Forces_ML || Prior_mode, adjust_white_advantage,
                  adjust_draw_rate, Anchor_use, Anchor_err_rel2avg

                  ,
                  General_average, Anchor, Priored_n, BETA

                  ,
                  &Encounters, &RPset, &Players, &RA, &Games

                  ,
                  PP, Wa_prior, Dr_prior

                  ,
                  &White_advantage, &Drawrate_evenmatch

      );

  ratings_results(Anchor_err_rel2avg, Anchor_use, Anchor, General_average,
                  &Players, &RA);
  ...
  if (Simulate > 1) {
    ...
    simul_smp(cpus, Simulate, sim_updates, quiet_mode, Forces_ML || Prior_mode,
              adjust_white_advantage, adjust_draw_rate, Anchor_use,
              Anchor_err_rel2avg
              ...
              ,
              drawrate_evenmatch_result, white_advantage_result, &RPset, PP,
              Wa_prior, Dr_prior

              ,
              &Encounters, &Players, &RA, &Games

              ,
              RPset_store, PP_store
  ...
  all_report(&Games, &Players, &RA, &RPset, &Encounters, sfe.sdev, Simulate,
             Hide_old_ver, Confidence_factor, csvf, textf,
             white_advantage_result, drawrate_evenmatch_result,
             decimals_array_n > 0 ? decimals_array[0] : 1  // OUTDECIMALS
             ,
             decimals_array_n > 1 ? decimals_array[1] : 1  // OUTDECIMALS
             ,
             outqual, sfe.wa_sdev, sfe.dr_sdev, sfe.relative, cfs_column,
             columns);
  ...
  if (head2head_str != NULL) {
    ...
    head2head_output(&Games, &Players, &RA, &Encounters, sfe.sdev, Simulate,
                     Confidence_factor, &Game_stats, sfe.relative,
                     head2head_str, OUTDECIMALS);
  }

  if (Elostat_output) {
    timelog("output in elostat format...");
    cegt_output(quiet_mode, &Games, &Players, &RA, &Encounters, sfe.sdev,
                Simulate, Confidence_factor, &Game_stats, sfe.relative, outqual,
  ...
  if (pdaba != NULL) database_done(pdaba);
  ...
  if (relstr != NULL) relpriors_done2(&RPset, &RPset_store);
...
static void example(void) {
  printf(
      ...
      "quick example: %s %s\n"
      "%s",
      proginfo_name(), example_options, example_str);
  return;
}
...
static void table_output(double rtng_76) {
  ...
  for (p = 50; p < 60; p++) {
    for (h = 0; h < 50; h += 10) {
      printf("%3d%6.1f   ", p + h,
             (p + h) == 50 ? 0 : inv_xpect(invbeta, (double)(p + h)));
    }
    printf("\n");
  }
  for (p = 0; p < 58; p++) {
