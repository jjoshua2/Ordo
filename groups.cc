...
#ifndef NDEBUG
static bool_t groupset_sanity_check(group_var_t* gv) {
...
static bool_t groupset_sanity_check_nocombines(group_var_t* gv) {
  // verify g is properly double-linked
  group_t* g = gv->groupbuffer.prehead;
  if (g == NULL) return FALSE;
  for (g = g->next; g != NULL; g = g->next) {
...
static void groupset_init(group_var_t* gv) {
...
static group_t* groupset_head(group_var_t* gv) {
  return gv->groupbuffer.prehead->next;
}
...
static void groupset_add(group_var_t* gv, group_t* a) {
...
static void groupset_reset_finding(group_var_t* gv) {
  group_t* s;
  player_t i;
...
static group_t* groupset_find(group_var_t* gv, player_t id) {
#if 1
...
static bool_t supporting_encmem_init(gamesnum_t nenc, super_t* sp,
                                     selink_t** pse) {
  struct ENC* a;
  selink_t* b;
  ...
  if (NULL == (a = (ENC*)memnew(sizeof(struct ENC) * (size_t)nenc))) {
    return FALSE;
  } else if (NULL == (b = (selink_t*)memnew(sizeof(selink_t) * (size_t)nenc))) {
    memrel(a);
    return FALSE;
  }
  sp->SE2 = a;
...
static void supporting_encmem_done(super_t* sp, selink_t* se) {
...
bool_t groupvar_init(group_var_t* gv, player_t nplayers, gamesnum_t nenc) {
  player_t* a;
  player_t* b;
  groupcell_t* c;
  node_t* d;
  ...
  if (NULL == (a = (player_t*)memnew(sa * (size_t)nplayers))) {
    return FALSE;
  } else if (NULL == (b = (player_t*)memnew(sb * (size_t)nplayers))) {
    memrel(a);
    return FALSE;
  } else if (NULL == (c = (groupcell_t*)memnew(sc * (size_t)nplayers))) {
    memrel(a);
    memrel(b);
    return FALSE;
  } else if (NULL == (d = (node_t*)memnew(sd * (size_t)nplayers))) {
    memrel(a);
    memrel(b);
    memrel(c);
    return FALSE;
  } else if (NULL == (e = (player_t*)memnew(se * (size_t)nplayers))) {
    memrel(a);
    memrel(b);
    memrel(c);
    memrel(d);
    return FALSE;
  } else if (NULL == (f = (group_t**)memnew(sf * (size_t)nplayers))) {
    memrel(a);
    memrel(b);
    memrel(c);
    memrel(d);
...
void groupvar_done(group_var_t* gv) {
  assert(gv);
...
static bool_t participant_buffer_init(struct PARTICIPANT_BUFFER* x,
                                      player_t n) {
  participant_t* p;
...
static group_t* group_combined(group_t* g) {
  while (g->combined != NULL) g = g->combined;
  return g;
}

// no globals
static void add_participant(group_var_t* gv, group_t* g, player_t i,
                            const char* name) {
  participant_t* nw;
...
// no globals
static void add_beat_connection(group_var_t* gv, group_t* g,
                                struct NODE* pnode) {
...
// no globals
static void add_lost_connection(group_var_t* gv, group_t* g,
                                struct NODE* pnode) {
  // Assume scan counters properly sorted and eliminated duplicates
  // otherwise, duplicates could be added
...
static void pnode_clear(pnode_t* pn, player_t n) {
  player_t i;
  for (i = 0; i < n; i++) {
    pn[i].indx = i;
    pn[i].next = NULL;
...
static void pnode_connect(pnode_t* pn, player_t i, player_t j) {
  player_t z;
  pnode_t *a, *b, *c, *l;
...
static bool_t same_SE_link(selink_t* a, selink_t* b) {
  return a->iwin == b->iwin && a->ilos == b->ilos;
}

// no globals
static void scan_encounters(const struct ENC* enc, gamesnum_t n_enc,
                            player_t* belongto, player_t n_plyrs,
                            struct ENC* sup_enc, gamesnum_t* pn_enc

                            ,
                            selink_t* sel, gamesnum_t* psel_n) {
  ...
  for (e = 0, n_b = 0; e < n_a; e++) {
    ...
    if (belongto[x] != belongto[y]) {
      if (e != n_b) sup_enc[n_b] = sup_enc[e];
      n_b++;
    }
  }

  *pn_enc =
      n_b;  // number of "Super" encounters that do not belong to the same group
...
static void node_add_group(group_var_t* gv, player_t x) {
  group_t* g;
...
static player_t group_belonging(group_var_t* gv, player_t x) {
...
// no globals
static void selink2group(player_t iwin, player_t ilos, group_var_t* gv) {
...
static void group_gocombine(group_t* g, group_t* h);

static void convert_general_init(group_var_t* gv, player_t n_plyrs) {
  player_t i;
...
static player_t groupvar_counter(group_var_t* gv) {
  return gv->groupfinallist_n;
}

static void groupvar_finallist_sort(group_var_t* gv);

player_t groupvar_build(group_var_t* gv, player_t n_plyrs, const char** name,
  ...
  if (supporting_encmem_init(encounters->n, sp, &SElnk)) {
    timelog("scan games...");
    scan_encounters(encounters->enc, encounters->n, gv->groupbelong, players->n,
                    sp->SE2, &sp->N_se2, SElnk, &SElnk_n);
...
static void groupvar_sieveenc(group_var_t* gv, const struct ENC* enc,
                              gamesnum_t n_enc, gamesnum_t* N_enca,
                              gamesnum_t* N_encb) {
  gamesnum_t e;
  player_t w, b;
...
static player_t id_pointed_by_conn(connection_t* c) {
  group_t* gg;
  return NULL != c && NULL != (gg = group_pointed_by_conn(c)) ? gg->id : NO_ID;
}

static player_t find_top_id(group_t* g) {
  connection_t* c;
  player_t id;
  player_t topid = g->id;
...
static void simplify_shrink__(group_t* g) {
  bitarray_t bA;
...
static void simplify_shrink_redundancy(group_t* g) {
#if 0
	printf("-------------\n");
...
static player_t find_combine_candidate(group_t* g, group_t* comb[]) {
...
static void simplify(group_t* g, group_t** combbuf) {
  player_t cmb;
  player_t n;
...
static group_t* group_unlink(group_t* g) {
  group_t *a, *b;
  assert(g);
  a = g->prev;
  b = g->next;
...
static group_t* group_next_pointed_by_beat(group_t* g) {
  group_t* gp;
  connection_t* c;
  player_t own_id;
  if (NULL == g || NULL == (c = group_beat_head(g))) return NULL;
  own_id = g->id;
  while (c && (NULL == (gp = group_pointed_by_conn(c)) || gp->isolated ||
               gp->id == own_id)) {
    c = beat_next(c);
  }
...
static player_t group_population(group_t* s) {
  assert(s);
  return participants_list_population(s->pstart);
}

static bool_t sorted(participant_t* a, participant_t* b);

static int compare_cell(const void* a, const void* b) {
  int r;
  const groupcell_t* ap = (const groupcell_t*)a;
  const groupcell_t* bp = (const groupcell_t*)b;
...
static void groupvar_finallist_sort(group_var_t* gv) {
  qsort(gv->groupfinallist, (size_t)gv->groupfinallist_n, sizeof(groupcell_t),
        compare_cell);
}
...
static void groupvar_finish(group_var_t* gv) {
    ...
    do {
      ...
      *chain++ = own_id;
      ...
      if (prev_g->lstart == NULL ||
          g == NULL) {  // no losses or no victories, quit now to optimize
  ...
  //	qsort (gv->groupfinallist, (size_t)gv->groupfinallist_n,
  //sizeof(groupcell_t), compare_cell);
...
static participant_t* plist_go_last(participant_t* pstart) {
  participant_t *s, *p;
  s = pstart;
  p = NULL;
  while (s) {
...
static participant_t* plist_inssort(participant_t* pstart) {
  participant_t head;
  ...
  while (i) {
    ...
    p = h;
    while (p->next && !sorted(i, p->next)) p = p->next;
...
static bool_t too_short(participant_t* a) {
  participant_t* p;
  long unsigned i = 0;
...
#if 1
static participant_t* get_half(participant_t* a) {
  participant_t *p, *s, *b;
  long unsigned i = 0;
...
static participant_t* merge(participant_t* a, participant_t* b) {
  participant_t head;
  participant_t *l, *t;
...
static participant_t* plist_mrgsort(participant_t* a) {
  participant_t *b, *r;

  if (too_short(a)) return plist_inssort(a);
...
static void participants_list_print(FILE* f, participant_t* pstart) {
  participant_t* p;
...
static void group_output(group_t* s, group_var_t* gv, FILE* f,
  ...
  if (winconnections == 0 && lossconnections == 0) {
    ...
    if (!showlinks)
      fprintf(f,
              " \\---> this group has incomplete links with the rest (only "
              "wins or losses)\n");
  }
}
...
static void groupvar_output_info(group_var_t* gv, FILE* groupf) {
  if (NULL != groupf) {
...
static void groupvar_to_groupid(group_var_t* gv, player_t* groupid_out) {
  if (groupid_out) {
    player_t i;
    for (i = 0; i < gv->nplayers; i++) {
...
group_var_t* GV_make(const struct ENCOUNTERS* encounters,
                     const struct PLAYERS* players) {
  player_t n = 0;
  bool_t ok = FALSE;
  group_var_t* gv;
...
group_var_t* GV_kill(group_var_t* gv) {
  assert(gv);
  groupvar_done(gv);
  memrel(gv);
  return NULL;
...
void GV_out(group_var_t* gv, FILE* f) {
  assert(f);
  if (f) groupvar_output_info(gv, f);
}

void GV_sieve(group_var_t* gv, const struct ENCOUNTERS* encounters,
              gamesnum_t* pN_intra, gamesnum_t* pN_inter) {
  assert(gv && encounters && pN_intra && pN_inter);
  groupvar_sieveenc(gv, encounters->enc, encounters->n, pN_intra, pN_inter);
}

player_t GV_counter(group_var_t* gv) {
  assert(gv);
  return groupvar_counter(gv);
}

void GV_groupid(group_var_t* gv, player_t* groupid_out) {
  assert(gv && groupid_out);
  groupvar_to_groupid(gv, groupid_out);
}

static player_t participants_list_actives(participant_t* pstart,
                                          const struct PLAYERS* players) {
  participant_t* p;
  player_t j;
  player_t accum;
...
static player_t group_number_of_actives(group_t* s,
                                        const struct PLAYERS* players) {
  return participants_list_actives(s->pstart, players);
}

player_t GV_non_empty_groups_pop(group_var_t* gv,
                                 const struct PLAYERS* players) {
  group_t* g;
  player_t i;
  player_t x;
...
bool_t well_connected(const struct ENCOUNTERS* pEncounters,
