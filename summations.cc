...
static double get_sdev(double s1, double s2, double n) {
  double xx = n * s2 - s1 * s1;
  xx = sqrt(xx * xx);  // removes problems with -0.00000000000;
  return sqrt(xx) / n;
}

static void summations_clear(struct summations* sm, player_t nplayers) {
  ptrdiff_t np = (ptrdiff_t)nplayers;
  ptrdiff_t est =
      (ptrdiff_t)((np * np - np) / 2); /* elements of simulation table */
  ptrdiff_t i, idx;
...
bool_t summations_calloc(struct summations* sm, player_t nplayers) {
  ptrdiff_t np = (ptrdiff_t)nplayers;
  ptrdiff_t est =
      (ptrdiff_t)((np * np - np) / 2); /* elements of simulation table */
...
void summations_init(struct summations* sm) {
  assert(sm);
  sm->relative = NULL;
  sm->sum1 = NULL;
  sm->sum2 = NULL;
...
void summations_done(struct summations* sm) {
  assert(sm);
...
void summations_update(struct summations* sm, player_t topn, double* ratingof,
                       double white_advantage, double drawrate_evenmatch) {
  player_t i, j;
  ptrdiff_t idx;
  double diff;
...
void summations_calc_sdev(struct summations* sm, player_t topn, double sim_n) {
  player_t i;
  ptrdiff_t est =
      (ptrdiff_t)((topn * topn - topn) / 2); /* elements of simulation table */
  ...
  for (i = 0; i < est; i++) {
    sm->relative[i].sdev =
        get_sdev(sm->relative[i].sum1, sm->relative[i].sum2, sim_n);
  }
