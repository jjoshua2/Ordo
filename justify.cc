...
static bool_t iswordbreak(int c) { return c == '\0' || isspace(c); }

static const char* goto_nextstop(const char* p, ptrdiff_t rem) {
  const char* q = p;
...
static const char* print_stretch(FILE* f, const char* sta, const char* end) {
  while (*sta && sta < end) {
    fprintf(f, "%c", *sta++);
  }
  return sta;
...
static bool_t fit_horizontally(const char* p, ptrdiff_t HOR) {
  ptrdiff_t i;
  bool_t found = FALSE;
...
static void carriage(FILE* f, int m) {
  while (m-- > 0) fprintf(f, " ");
}

static void carriage_return(FILE* f, int m) {
  fprintf(f, "\n");
  carriage(f, m);
}
...
void fprint_justified(FILE* f, const char* p, int blankheader, int margin,
                      int hor) {
  const char* q;
  ptrdiff_t rem, HOR;
...
void print_justified(const char* p, int blankheader, int margin, int hor) {
