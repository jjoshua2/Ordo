...
static gamesnum_t calc_encounters(int selectivity, const struct GAMES* g,
                                  const bool_t* flagged, struct ENC* enc  // out
);

//-----------------------------------------------------------------------

bool_t encounters_init(gamesnum_t n, struct ENCOUNTERS* e) {
  struct ENC* p;
...
void encounters_done(struct ENCOUNTERS* e) {
  memrel(e->enc);
  e->n = 0;
  e->size = 0;
  e->enc = NULL;
...
void encounters_copy(const struct ENCOUNTERS* src, struct ENCOUNTERS* tgt) {
...
bool_t encounters_replicate(const struct ENCOUNTERS* src,
                            struct ENCOUNTERS* tgt) {
  bool_t ok;
...
void encounters_calculate(int selectivity, const struct GAMES* g,
                          const bool_t* flagged, struct ENCOUNTERS* e) {
  e->n = calc_encounters(selectivity, g, flagged, e->enc);
}
...
static gamesnum_t calc_encounters(int selectivity, const struct GAMES* g,
  ...
  for (i = 0; i < n_games; i++) {
    ...
    skip = score_i >= DISCARD || (selectivity == ENCOUNTERS_NOFLAGGED &&
...
// no globals
void calc_obtained_playedby(const struct ENC* enc, gamesnum_t N_enc,
                            player_t n_players, double* obtained,
                            gamesnum_t* playedby) {
  player_t w, b;
  player_t j;
  gamesnum_t e;
...
#if !defined(NDEBUG)
bool_t ratings_sanity(player_t n_players, const double* ratingof) {
  player_t j;
...
bool_t playedby_sanity(player_t n_players, const gamesnum_t* pb,
...
// no globals
void calc_expected(const struct ENC* enc, gamesnum_t N_enc,
                   double white_advantage, player_t n_players,
                   const double* ratingof, double* expected, double beta) {
  player_t w, b;
  player_t j;
  gamesnum_t e;
  double wperf;
  ...
  for (e = 0; e < N_enc; e++) {
    ...
    wperf = (double)enc[e].played *
            xpect(ratingof[w] + white_advantage, ratingof[b], beta);
    assert(!is_nan(wperf));
...
static player_t compact_plist(player_t* plist, player_t plist_n) {
  player_t r, i;
...
// no globals
void calc_output_info(const struct ENC* enc, gamesnum_t N_enc,
                      const double* ratingof, player_t n_players, double* sdev,
                      struct OUT_INFO* oi, player_t* sorted,
                      player_t sorted_n) {
  encnode_t** pend;
  encnode_t* ep;
  gamesnum_t* p__;
