...
void ba_put(struct BITARRAY* ba, player_t x) {
  assert(ba);
...
bool_t ba_ison(struct BITARRAY* ba, player_t x) {
  uint64_t y;
  bool_t ret;
  assert(ba);
  assert(ba->pod && ba->max);
...
void ba_clear(struct BITARRAY* ba) {
  size_t i;
  player_t max;
  size_t max_p;
...
void ba_setnot(struct BITARRAY* ba) {
  size_t i;
  player_t max;
  size_t max_p;
...
bool_t ba_init(struct BITARRAY* ba, player_t max) {
  bool_t ok;
  uint64_t* ptr;
  size_t i;
...
void ba_done(struct BITARRAY* ba) {
  assert(ba);
  assert(ba->pod);
